...
export default function ModelLab({ model, onExplored }: { model: ModelScene; onExplored: () => void }) {
  const [explode, setExplode] = useState(0), [playing, setPlaying] = useState(false), [speed, setSpeed] = useState(1), [labels, setLabels] = useState(true), [stage, setStage] = useState(0), [picked, setPicked] = useState(model.parts[0])
  useEffect(() => { setStage(0); setPicked(model.parts[0]); setExplode(0); setPlaying(false) }, [model.id])
  useEffect(() => {
    if (!playing) return
    if (stage >= model.stages.length - 1) { setPlaying(false); return }
    const timer = window.setTimeout(() => setStage(current => Math.min(current + 1, model.stages.length - 1)), Math.max(1400, 3600 / speed))
    return () => window.clearTimeout(timer)
  }, [model.id, model.stages.length, playing, speed, stage])
  const mark = (fn: () => void) => { fn(); onExplored() }
  const chooseStage = (next: number) => { setStage(next); setPlaying(false); onExplored() }
  const togglePlayback = () => mark(() => {
    if (playing) setPlaying(false)
    else { if (stage >= model.stages.length - 1) setStage(0); setPlaying(true) }
  })
  const stageNote = model.stageNotes?.[stage] || `观察“${model.stages[stage]}”中的结构变化，并与前后阶段比较。`
  return <section className="model-lab" aria-label={`${model.name}三维模型`}>
    ...
    <div className="stage-switcher" aria-label="模型场景选择">{model.stages.map((s, i) => <button aria-pressed={i === stage} className={i === stage ? 'active' : ''} key={s} onClick={() => chooseStage(i)}><span>{i + 1}</span><strong>{s}</strong></button>)}</div>
    <div className="stage-explanation" aria-live="polite"><div><span>本阶段观察</span><p>{stageNote}</p></div><strong>{String(stage + 1).padStart(2, '0')} / {String(model.stages.length).padStart(2, '0')}</strong><i><b style={{ width: `${(stage + 1) / model.stages.length * 100}%` }} /></i></div>
    <div className="model-controls"><button className="ghost small step-control" disabled={stage === 0} onClick={() => chooseStage(stage - 1)}>← 上一步</button><button className="primary small play-control" onClick={togglePlayback}>{playing ? '暂停动画' : stage === model.stages.length - 1 ? '重新播放' : '播放全过程'}</button><button className="ghost small step-control" disabled={stage === model.stages.length - 1} onClick={() => chooseStage(stage + 1)}>下一步 →</button><label>拆解 <input aria-label="拆解程度" type="range" min="0" max="1" step=".05" value={explode} onChange={e => mark(() => setExplode(+e.target.value))} /></label><label>速度 <select aria-label="动画速度" value={speed} onChange={e => setSpeed(+e.target.value)}><option value=".5">0.5×</option><option value="1">1×</option><option value="2">2×</option></select></label><button className="ghost small" onClick={() => setLabels(!labels)}>{labels ? '隐藏标注' : '显示标注'}</button>{model.compare && <span className="compare-chip">对比：{model.compare.join(' ↔ ')}</span>}</div>
