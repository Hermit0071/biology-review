...
import { createDefaultSettings, migrateSettings } from './examDate'
...
function fallbackRead(): FallbackState {
  try {
    ...
    return value ? JSON.parse(value) as FallbackState : { progress: [], sessions: [], settings: createDefaultSettings() }
  } catch { return { progress: [], sessions: [], settings: createDefaultSettings() } }
}
...
export const storage = {
  ...
  async settings(): Promise<AppSettings> {
    try {
      const db = await openDB()
      const saved = await new Promise<AppSettings>((resolve) => {
        const req = db.transaction('settings', 'readonly').objectStore('settings').get('main')
        req.onsuccess = () => resolve((req.result as AppSettings) || createDefaultSettings())
        req.onerror = () => resolve(createDefaultSettings())
      })
      const migrated = migrateSettings(saved)
      if (migrated.examDate !== saved.examDate || migrated.schemaVersion !== saved.schemaVersion) await put('settings', migrated, 'main')
      return migrated
    } catch {
      const data = fallbackRead()
      const migrated = migrateSettings(data.settings)
      if (migrated.examDate !== data.settings.examDate || migrated.schemaVersion !== data.settings.schemaVersion) fallbackWrite({...data, settings:migrated})
      return migrated
    }
  },
